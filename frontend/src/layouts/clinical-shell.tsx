    ...
    <div className="h-screen flex flex-col md:flex-row overflow-hidden bg-canvas text-ink">
      ...
      <aside
        className={cn(
          ...
          'w-[250px] bg-white border-r border-[var(--color-border)] shadow-xs',
      ...
      >
        ...
        <div className="p-4 border-b border-[var(--color-border)] bg-white">
          <div className="flex items-center justify-between">
            ...
            <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[10px] font-bold tracking-wider uppercase bg-[var(--color-verified-subtle)] text-[var(--color-verified-text)] border border-emerald-200/60">
          ...
          <div className="mt-3.5 p-3 bg-[var(--color-surface-subtle)] rounded-xl border border-[var(--color-border)]">
            <div className="flex items-center gap-2.5">
              <div
                className={cn(
                  'w-8 h-8 rounded-lg flex items-center justify-center shrink-0 text-white font-bold text-[12px] shadow-2xs',
                  isDoctor ? 'bg-[var(--color-brand)]' : 'bg-[var(--color-warning)]'
                )}
              >
                {isDoctor ? <Stethoscope size={16} /> : <Activity size={16} />}
              </div>
              <div className="min-w-0 flex-1">
                <span className="text-[12.5px] font-bold text-[var(--color-text-primary)] block truncate leading-tight">
                  {stationTitle}
                </span>
                <span className="text-[11px] text-[var(--color-text-muted)] block truncate font-medium">
                  {departmentName}
                </span>
              </div>
            </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1.5 overflow-y-auto" role="navigation">
          <div className="px-3 pb-1 text-[10.5px] font-bold uppercase tracking-wider text-[var(--color-text-muted)]">
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    active
                      ? 'bg-[var(--color-brand-mist)] text-[var(--color-brand)] font-bold'
                      : 'text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-subtle)] hover:text-[var(--color-text-primary)] font-medium'
                  )}
                >
                  {/* Left Active Indicator Line */}
                  {active && (
                    <span className="absolute left-0 top-1.5 bottom-1.5 w-1 bg-[var(--color-brand)] rounded-r-sm" />
                  )}
                  <Icon
                    size={15}
                    className={cn(
          ...
          <div className="pt-4 mt-4 border-t border-[var(--color-border)]">
            ...
            <Link
              href="/kiosk"
              target="_blank"
              className="flex items-center gap-2 px-3 py-2 rounded-lg text-[12px] text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-subtle)] hover:text-[var(--color-brand)] transition-colors group"
            >
              <ExternalLink size={13} className="text-[var(--color-text-muted)] group-hover:text-[var(--color-brand)]" />
              <span className="flex-1 truncate">Patient Intake Kiosk</span>
              <span className="text-[10px] font-mono bg-white px-1.5 py-0.5 rounded border border-[var(--color-border)] font-bold text-[var(--color-text-muted)]">
                Launch
              </span>
            </Link>
          </div>
        ...
        <div className="p-3.5 border-t border-[var(--color-border)] bg-white space-y-2">
          <div className="flex items-center gap-2 p-2 rounded-xl bg-[var(--color-surface-subtle)] border border-[var(--color-border)]">
            <div
              className={cn(
                'w-7 h-7 rounded-lg text-white flex items-center justify-center text-[11px] font-bold shrink-0 shadow-2xs',
                isDoctor ? 'bg-[var(--color-brand)]' : 'bg-[var(--color-warning)]'
              )}
            >
              {isDoctor ? 'DR' : 'RN'}
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[12px] font-bold truncate text-[var(--color-text-primary)] leading-tight">
                {user?.name || (isDoctor ? 'Dr. Sunita Rao, MD' : 'Sister Anita Sharma')}
              </p>
              <p className="text-[10.5px] truncate text-[var(--color-text-muted)]">
                {roleTitle}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between text-[11px] px-1 pt-1">
            <Link
              href="/"
              className="text-[var(--color-text-secondary)] hover:text-[var(--color-brand)] font-medium transition-colors"
            >
              ← Public Home
            </Link>
            <button
              onClick={logout}
              className="flex items-center gap-1 text-[var(--color-critical)] hover:underline font-bold transition-colors cursor-pointer"
            >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-13 flex items-center justify-between px-4 md:px-6 shrink-0 bg-white border-b border-[var(--color-border)] shadow-2xs">
          <div className="flex items-center gap-3">
            <button
              className="md:hidden p-1.5 rounded-lg text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-subtle)] transition-colors"
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[var(--color-verified-subtle)] border border-emerald-200/60 text-[11px] font-bold text-[var(--color-verified-text)]">
              <ShieldCheck size={12} className="text-[var(--color-verified)]" />
              <span>ABDM FHIR R4 Ready</span>
            </div>

            <div className="flex items-center gap-1.5 px-2.5 py-0.8 rounded-full bg-[var(--color-surface-subtle)] border border-[var(--color-border)] text-[11.5px] font-mono font-bold text-[var(--color-text-primary)]">
