    ...
    >
      <div className="max-w-[1440px] mx-auto px-6 py-4 flex items-center justify-between gap-4">
        ...
        <div className="flex flex-col leading-none">
          <time
            className="text-[22px] font-bold font-mono tracking-wider text-white"
            aria-label={`Current time: ${time}`}
          >
            {time}
          </time>
          <span className="text-[12px] text-white/70 mt-0.5 font-medium">
            {date}
          </span>
        </div>
        ...
        {currentToken !== undefined && (
          <div className="flex items-center gap-2 bg-white/15 px-4 py-2 rounded-xl border border-white/20">
            <div className="w-2 h-2 rounded-full bg-[var(--color-verified)] animate-pulse" aria-hidden="true" />
